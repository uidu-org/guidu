...
type PopperChildren = any; // { children: PopperChildrenProps => Node };
type PopperPropsNoChildren = any; // $Diff<PopperProps, PopperChildren>;
type Props = {
  ...
  styles: Object;
  target: ({ ref, isOpen }: { ref: any; isOpen: boolean }) => any;
};
...
export default class PopupSelect extends PureComponent<Props, State> {
  focusTrap: Object;
  menuRef: HTMLElement;
  selectRef: React.RefObject<any> = React.createRef();
  ...
  open = () => {
    ...
    this.selectRef.current.select.focusOption('first'); // HACK
  ...
  close = () => {
    ...
    (this.focusTrap as any).deactivate();
  ...
  resolveTargetRef = (popperRef: any) => (ref: HTMLElement) => {
  ...
  resolveMenuRef = (popperRef: any) => (ref: HTMLElement) => {
    this.menuRef = ref;
    popperRef(ref);
  };
  ...
  getItemCount = () => {
    ...
    options.forEach((groupOrOption: any) => {
  ...
  getMaxHeight = () => {
    ...
    const offsetHeight = showSearchControl
      ? this.selectRef.current.select.controlRef.offsetHeight
      : 0;
  ...
  renderSelect = () => {
    ...
    return (mergedPopperProps as any).positionFixed
      ? popper
