...
const exampleURL = getExampleUrl('core', 'navigation-next', 'composed-navigation');
/* Css selectors used for the tests */

const getByKey = key => `[data-webdriver-test-key="${key}"]`;

BrowserTestCase('navigation.js: 🌏 Global navigation should render without errors', async client => {
  const page = new Page(client);
  const globalNav = getByKey('global-navigation');
  await page.goto(exampleURL);
  expect((await page.isVisible(`${globalNav} [aria-label="Jira"]`))).toBe(true);
  expect((await page.isVisible(`${globalNav} [aria-label="Search"]`))).toBe(true);
...
BrowserTestCase('navigation.js: 📦 Product navigation should render without errors', async client => {
  const page = new Page(client);
  await page.goto(exampleURL);
  expect((await page.isVisible(getByKey('product-header')))).toBe(true);
  expect((await page.isVisible(getByKey('product-item-dashboards')))).toBe(true);
...
BrowserTestCase('navigation.js: 🎁 Container navigation should render without errors', async client => {
  const page = new Page(client);
  await page.goto(exampleURL);
